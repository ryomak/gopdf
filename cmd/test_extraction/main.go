...
func main() {
	...
	// Store elements for later analysis
	var allElements []gopdf.TextElement
	...
	for pageNum := 0; pageNum < pageCount; pageNum++ {
		...
		if pageNum == 0 {
			allElements = elements // Save for debug output
		}
		if err != nil {
	...
	// Debug: Show hex codes from content stream
	fmt.Println("\n--- Debug: Type3 Font Elements (F9-F19) ---")
	count := 0
	for i, elem := range allElements {
		if elem.Font == "F9" || elem.Font == "F10" || elem.Font == "F11" ||
			elem.Font == "F12" || elem.Font == "F13" || elem.Font == "F14" ||
			elem.Font == "F15" || elem.Font == "F16" || elem.Font == "F17" ||
			elem.Font == "F18" || elem.Font == "F19" {
			fmt.Printf("Element[%d]: Font=%s, Text=%q, ", i, elem.Font, elem.Text)
			fmt.Print("Runes=")
			for _, r := range elem.Text {
				fmt.Printf("U+%04X ", r)
			}
	...
	if count == 0 {
		fmt.Println("  No Type3 font elements found")
	}
