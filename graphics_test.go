...
func TestPredefinedColors(t *testing.T) {
	if ColorBlack.R != 0 || ColorBlack.G != 0 || ColorBlack.B != 0 {
		t.Error("ColorBlack color is incorrect")
	}
	if ColorWhite.R != 1 || ColorWhite.G != 1 || ColorWhite.B != 1 {
		t.Error("ColorWhite color is incorrect")
	}
	if ColorRed.R != 1 || ColorRed.G != 0 || ColorRed.B != 0 {
		t.Error("ColorRed color is incorrect")
	}
	if ColorGreen.R != 0 || ColorGreen.G != 1 || ColorGreen.B != 0 {
		t.Error("ColorGreen color is incorrect")
	}
	if ColorBlue.R != 0 || ColorBlue.G != 0 || ColorBlue.B != 1 {
		t.Error("ColorBlue color is incorrect")
	}
}
