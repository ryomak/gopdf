...
日本語テキストに対してふりがな（ルビ）を付与し、視覚表示とコピー動作を制御する機能。
• FR-8.1: ルビ付きテキスト描画:
    ◦ 漢字の上（または下）に小さくふりがなを配置できる。
    ◦ モノルビ（一文字ずつルビを振る）とグループルビ（熟語全体にルビを振る）の両方に対応する。
    ◦ ルビのフォントサイズ、位置（上付き/下付き）、オフセット（親文字との距離）をカスタマイズできる。
    ◦ 縦書き・横書きの両方でルビを正しく配置する。
• FR-8.2: コピー動作のカスタマイズ:
    ◦ PDFのActualText属性を使用して、表示内容とコピー時のテキストを分離する。
    ◦ コピー時は漢字のみを取得（デフォルト動作）。例: 表示「東京（とうきょう）」→ コピー「東京」
    ◦ コピー時はひらがなのみを取得（学習教材向け）。例: 表示「東京（とうきょう）」→ コピー「とうきょう」
    ◦ コピー時は「漢字(かんじ)」形式を取得（読みを含める）。例: 表示「東京（とうきょう）」→ コピー「東京(とうきょう)」
    ◦ コピー動作の設定を文字列単位、段落単位、ドキュメント全体で指定可能。
• FR-8.3: 一括ルビ振り:
    ◦ テキストと読みがなのペア配列を渡すだけで、自動的にレイアウトを計算してルビを配置する。
    ◦ 辞書ファイル（CSV、JSON形式）からルビ情報を読み込み、自動的にルビを振る機能をサポートする（オプション）。
    ◦ ルビ付きテキストのプレビュー機能（ルビの配置確認用）。

FR-9: セキュリティ機能（パスワード保護・暗号化）

PDFドキュメントのセキュリティを確保し、パスワード保護や暗号化を行う機能。
• FR-9.1: パスワード保護:
    ◦ ユーザーパスワード（文書を開くためのパスワード）を設定できる。
    ◦ オーナーパスワード（編集権限を制御するパスワード）を設定できる。
    ◦ パスワード強度の検証機能（最小文字数、複雑性チェック）を提供する。
• FR-9.2: 暗号化レベル:
    ◦ 40-bit RC4（後方互換性のため、非推奨）
    ◦ 128-bit RC4（標準的な暗号化）
    ◦ 128-bit AES（推奨）
    ◦ 256-bit AES（最も強力、推奨）
    ◦ デフォルトは256-bit AESとする。
• FR-9.3: アクセス権限の細かい制御:
    ◦ 印刷禁止/許可、および印刷品質の制限（低解像度のみ/高解像度）
    ◦ テキスト・画像のコピー禁止/許可
    ◦ ドキュメントの編集禁止/許可（コンテンツの追加・削除・変更）
    ◦ 注釈（コメント、マーカー）の追加禁止/許可
    ◦ フォームフィールドへの入力禁止/許可
    ◦ ページの抽出・挿入・削除・回転の禁止/許可
    ◦ スクリーンリーダー用のテキスト抽出は常に許可（アクセシビリティ配慮）
• FR-9.4: セキュリティプリセット:
    ◦ 「社外秘」プリセット（印刷・コピー・編集すべて禁止）
    ◦ 「閲覧のみ」プリセット（印刷許可、コピー・編集禁止）
    ◦ 「印刷可能」プリセット（印刷・閲覧許可、編集禁止）
    ◦ 「レビュー用」プリセット（閲覧・注釈追加許可、編集禁止）
    ◦ カスタムプリセットの定義・保存・再利用機能
• FR-9.5: パスワード保護されたPDFの読み込み:
    ◦ ユーザーパスワードを指定してパスワード保護されたPDFを開く。
    ◦ オーナーパスワードを指定してPDFを開く（編集権限の取得）。
    ◦ パスワードが間違っている場合、適切なエラーメッセージを返す。
    ◦ 暗号化方式（RC4/AES、ビット長）の自動検出。
• FR-9.6: パスワード保護の解除:
    ◦ オーナーパスワードを使用して、暗号化を完全に解除した新しいPDFを生成できる。
    ◦ アクセス権限のみを解除し、パスワード保護は維持する（部分的な解除）。
    ◦ 解除操作のログ記録（いつ、誰が、どのファイルに対して実行したか）。
• FR-9.7: パスワードリカバリー支援（正当な用途限定）:
    ◦ 弱いパスワードの検出機能（よくあるパスワード、数字のみ、短いパスワード）
    ◦ ユーザー指定の辞書ファイルを使用した辞書攻撃
    ◦ パターンベースのパスワード推測（日付形式、名前＋数字など）
    ◦ 試行回数の制限機能（デフォルト1000回、カスタマイズ可能）
    ◦ 処理時間の上限設定（タイムアウト機能）
    ◦ 実行ログの記録（監査証跡）
    ◦ 倫理的配慮: ドキュメントに「自己所有または正当な権限のあるファイルにのみ使用すること」の警告を明記
• FR-9.8: セキュリティ診断:
    ◦ PDFファイルの暗号化強度を分析（暗号化方式、ビット長、権限設定）
    ◦ 弱いパスワードやセキュリティ設定の警告
    ◦ 推奨セキュリティ設定の提示
    ◦ セキュリティ診断レポートの生成（監査用、PDF形式）
• FR-9.9: バッチ処理:
    ◦ フォルダ内の複数PDFに対して一括でパスワード設定
    ◦ 同じパスワードで保護された複数ファイルの一括パスワード解除
    ◦ バッチ処理の進捗状況表示
    ◦ 並列処理によるパフォーマンス向上
